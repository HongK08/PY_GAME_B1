...
#############################################################################
    ...
    import pygame, pygame_menu  # Pygame과 Pygame_menu 모듈을 불러옵니다.
    
    pygame.init()  # Pygame 초기화
    
    surface = pygame.display.set_mode((600, 400))  # 게임 화면 크기 설정 (600x400)
    
    def level(self, value):  # 난이도 선택 시 호출되는 함수
        print("난이도 선택값:", value)  # 선택된 난이도 값 출력 (현재는 단순 출력)
    
    def start():  # 게임 시작 선택 시 호출되는 함수
        print("게임시작")  # 게임 시작 메시지 출력 (실제 게임 로직은 여기에 추가)
    ...
    # 메뉴 생성: 제목 "Menu", 크기 400x300, 테마 t
    menu = pygame_menu.Menu("Menu", 400, 300, theme=t)
    
    # 메뉴 항목 추가: 난이도 선택 (Hard, Easy)
